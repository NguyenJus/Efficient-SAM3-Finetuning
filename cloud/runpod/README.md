...
We deliberately do **not** publish or maintain a custom RunPod image — see [issue #34](https://github.com/NguyenJus/Efficient-SAM3-Finetuning/issues/34). Use the stock **"RunPod PyTorch 2.x"** template from the Templates page. Templates → Deploy → pick the GPU from step 2 → Deploy.

## 4. Set `HF_TOKEN`

Pod → Edit → Environment Variables → add `HF_TOKEN` (your Hugging Face read-access token for gated `facebook/sam3.1`). **Or** skip this if you've mounted a network volume that contains `models/sam3.1/sam3.1_multiplex.pt` — `esam3` will detect the local file and skip HF auth.
...
Two options: drag and drop the notebook file into the Jupyter file browser, or in Jupyter, File → Open from URL → paste the raw GitHub URL for `notebooks/esam3_train.ipynb`.
